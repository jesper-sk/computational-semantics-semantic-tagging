...
class DecisionTreeTagger:
    ...
    def __init__(self, options: DecisionTreeTaggerOptions = None) -> None:
        self.__options = options or DecisionTreeTaggerOptions() # take defaults if no options given
        self.__model = None
    ...
    def train(self, input_data: str) -> None:
        ...
        if self.__options.load_pretrained:
            if self.load_model():
                print("Found pretrained decision tree model. Skipping training (use --force-train to force training).")
                return
            else:
                print("No pretrained decision tree model found, training now...")
        ...
        if self.__options.use_grid_search:
            ...
            dtree = GridSearchCV(tree.DecisionTreeClassifier(), params, n_jobs=4)
